...
module.exports = grammar({
  ...
  rules: {
    ...
    _environment_variable: (_) =>
      token(seq("{$", token.immediate(/[a-zA-Z0-9][a-zA-Z0-9_.\[\]]*/), "}")),

    // Directives
    directive_name: (_) => /[a-zA-Z_\-+]+/,
    directive: ($) =>
      seq(field("name", $.directive_name), ...directiveFields($)),

    // https://caddyserver.com/docs/caddyfile/matchers#named-matchers
    matcher_name: (_) => token(seq("@", /[a-zA-Z0-9\-_]+/)),
    named_matcher: ($) =>
      seq(field("name", $.matcher_name), ...subdirectiveFields($)),
    matcher: ($) =>
      choice(
        // Allow a lone `*`
        "*",
    ...
    single_site: ($) =>
      seq(
        field("name", commaSep1($.site_address)),
        field("definitions", repeat($._definition)),
      ),

    site_definition: ($) =>
      seq(field("name", commaSep1($.site_address)), $.block),
    ...
    unix_socket: (_) =>
      token(prec(2, seq(/[a-z][a-z+]*/, "//", /[a-zA-Z0-9_\-./*]+/))),
      ...
      choice(
        ...
        token(
          prec(
            ...
            seq(
              optional(URL_SCHEME_REGEX),
              choice(IPV4_REGEX, seq("[", IPV6_REGEX, "]"), HOSTNAME_REGEX),
              optional(seq(":", PORT_REGEX)),
            ),
          ),
        ),
